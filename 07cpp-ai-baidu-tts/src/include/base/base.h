...
private:
  ...
  std::string vector_join(const std::vector<std::string> & v_images,
      VECTOR_JOIN_TYPE type) {
    std::string images;
...
protected:
  std::string getAccessToken() {
    ...
    int status_code = this->client.get(ACCESS_TOKEN_URL, &params, nullptr,
        &response);
...
public:
  ...
  AipBase(const std::string & app_id, const std::string & ak,
      const std::string & sk) :
      _app_id(app_id), _is_bce(false), ak(ak), sk(sk) {
    if (_app_id == "") {
    }
  }
  void set_is_bce() {
  ...
  Json::Value request(std::string url,
    ...
    int status_code = this->client.post(url, &temp_params, data,
        &headers_for_sign, &response);
