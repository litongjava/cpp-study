...
public:
  ...
  Speech(const std::string app_id, const std::string & ak,
      const std::string & sk) :
      AipBase(app_id, ak, sk) {
  }

  Json::Value request_asr(std::string const & url, Json::Value & data) {
    std::string response;
    Json::Value obj;
    ...
    reader->parse(response.data(), response.data() + response.size(), &obj,
        &error);

    return obj;
  }

  Json::Value request_tts(const std::string url,
      std::map<std::string, std::string> & data, std::string & file_content) {
    std::string response;
    Json::Value obj;
  ...
  Json::Value recognize(const std::string voice_binary,
      const std::string & format, const int & rate,
    ...
    data["speech"] = base64_encode(voice_binary.c_str(),
        (int) voice_binary.size());
    data["format"] = format;
  ...
  Json::Value recognize_url(const std::string & url,
      ...
      const int & rate, std::map<std::string, std::string> options) {
    Json::Value data;
  ...
  Json::Value text2audio(const std::string & text,
      std::map<std::string, std::string> const & options,
