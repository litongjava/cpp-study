...
public:
  ...
  Contentcensor(const std::string & app_id, const std::string & ak,
      const std::string & sk) :
   ...
   * options 可选参数:
   */
  Json::Value anti_porn_gif(std::string const & image,
    ...
    Json::Value result = this->request(_anti_porn_gif, null, data, null);
   ...
   * options 可选参数:
   */
  Json::Value anti_spam(std::string const & content,
      const std::map<std::string, std::string> & options) {
    std::map<std::string, std::string> data;
    ...
    Json::Value result = this->request(_anti_spam, null, data, null);

    return result;
  }

  Json::Value user_defined(std::string const & image_or_url,
      std::string const & type,
      const std::map<std::string, std::string> & options) {
    std::map<std::string, std::string> data;

    if (type == "image") {
      data["image"] = base64_encode(image_or_url.c_str(),
          (int) image_or_url.size());
    }
    if (type == "imgUrl") {
    ...
    Json::Value result = this->request(_user_defined, null, data, null);

    return result;
  }

  Json::Value combo(std::string const & image_or_url, std::string const & type,
      std::vector<std::string> const & scenes, Json::Value const & scene_conf) {
    Json::Value data;
    ...
    Json::Value result = this->request(_combo, null, data.toStyledString(),
        extern_headers);

    return result;
  }

  Json::Value face_audit(const std::vector<std::string> image_or_url,
      std::string const & type,
      const std::map<std::string, std::string> & options) {
    std::map<std::string, std::string> data;
    ...
    Json::Value result = this->request(_face_audit, null, data, null);
  ...
  Json::Value report(Json::Value const & report) {
    ...
    Json::Value result = this->request(_report, null, param.toStyledString(),
        null);
